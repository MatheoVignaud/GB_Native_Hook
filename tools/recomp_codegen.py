...
def render_c(funcs: List[FunctionDesc],
    ...
    out.append("static void recomp_print_usage(const char *argv0)")
    out.append("{")
    out.append("    const char *prog = (argv0 && argv0[0] != '\\0') ? argv0 : \"GB_Recompiled\";")
    out.append("    printf(\"Usage: %s [options] [rom_path]\\n\", prog);")
    out.append("    printf(\"\\n\");")
    out.append("    printf(\"Display:\\n\");")
    out.append("    printf(\"  -h, --help                 Show this help and exit\\n\");")
    out.append("    printf(\"  --display                  Enable SDL display window\\n\");")
    out.append("    printf(\"  --no-display               Disable SDL display window\\n\");")
    out.append("    printf(\"\\n\");")
    out.append("    printf(\"Validation / fuzz:\\n\");")
    out.append("    printf(\"  --fuzz-verify              Compare recompiled/native behavior on sampled keys\\n\");")
    out.append("    printf(\"  --fuzz-iters <n>           Iterations per key (default 64)\\n\");")
    out.append("    printf(\"  --fuzz-seed <n>            RNG seed for fuzzing\\n\");")
    out.append("    printf(\"  --fuzz-max-keys <n>        Limit number of keys fuzzed\\n\");")
    out.append("    printf(\"\\n\");")
    out.append("    printf(\"Shadow diff:\\n\");")
    out.append("    printf(\"  --shadow-diff              Enable runtime shadow diff checks\\n\");")
    out.append("    printf(\"  --shadow-diff-every <n>    Diff every N executions\\n\");")
    out.append("    printf(\"  --shadow-diff-max-reports <n>\\n\");")
    out.append("    printf(\"  --shadow-diff-stop         Stop on first mismatch\\n\");")
    out.append("    printf(\"  --shadow-quarantine        Quarantine mismatching functions\\n\");")
    out.append("    printf(\"\\n\");")
    out.append("    printf(\"Runtime learning:\\n\");")
    out.append("    printf(\"  --runtime-discover         Enable runtime function discovery\\n\");")
    out.append("    printf(\"  --runtime-read-trace       Record ROM reads\\n\");")
    out.append("    printf(\"  --runtime-read-trace-path <path>\\n\");")
    out.append("    printf(\"  --runtime-input-profile    Profile input-dependent reads\\n\");")
    out.append("    printf(\"  --runtime-input-snapshots  Save input snapshots/samples\\n\");")
    out.append("    printf(\"  --runtime-learn            Enable all runtime learning modes above\\n\");")
    out.append("    printf(\"\\n\");")
    out.append("    printf(\"Dispatch / trust:\\n\");")
    out.append("    printf(\"  --dynram / --no-dynram     Toggle dynram dispatch variants\\n\");")
    out.append("    printf(\"  --trust-only               Only execute trusted funcs list\\n\");")
    out.append("    printf(\"  --trusted-funcs <path>     Trusted functions file path\\n\");")
    out.append("    printf(\"\\n\");")
    out.append("    printf(\"Notes:\\n\");")
    out.append("    printf(\"  - If no ROM is provided, embedded assets or a default ROM may be used.\\n\");")
    out.append("    printf(\"  - Many options also have GB_RECOMP_* environment variable equivalents.\\n\");")
    out.append("}")
    out.append("")
    ...
    out.append("    {")
    out.append("        if (strcmp(argv[i], \"-h\") == 0 || strcmp(argv[i], \"--help\") == 0)")
    out.append("        {")
    out.append("            recomp_print_usage(argc > 0 ? argv[0] : \"GB_Recompiled\");")
    out.append("            return 0;")
    out.append("        }")
    out.append("        else if (strcmp(argv[i], \"--no-display\") == 0)")
